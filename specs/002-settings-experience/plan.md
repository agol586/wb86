...
实现采用纯 Swift schema v3 设置快照、显式 v1→v2→v3 迁移、按会话代次安全应用、状态化事件映射器，
会话内独立修饰键状态机，以及构建期编译、运行期只读内存映射的本地拼音资源。输入引擎以有序复合动作处理第五码，
候选查询以五笔优先、拼音随后、按显示文本去重；拼音精确键为空时通过固定键数和候选数上限执行
本地前缀预测；设置窗口只通过协调器发布完整有效快照，
二码/三码五笔查询复用 WB86 两字母 prefix range 追加以精确候选开头的本地词组，精确候选保持优先，
并在高级页集中提供私密模式与本地学习运行时控制，不创建额外状态栏项目。
基础词库保持完整；五笔和拼音分别排序并应用每类 64 项的既有安全上限后，默认“常用”策略以纯
Swift 固定 Unicode 范围过滤 Rime 定义的扩展 CJK，再按显示文本去重和分页。用户可在常用页开启
“显示扩展汉字”跳过过滤并恢复同一有界查询窗口内的候选；过滤不依赖系统字体或词频权重。
...
**Scale/Scope**: 40 项功能需求、四组设置页、约 21 个用户设置字段、2 个运行时隐私控制、5 组独立翻页键、3 类模式切换和一个本地连续全拼候选源；支持多个并发输入会话
