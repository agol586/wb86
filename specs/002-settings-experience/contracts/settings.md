...
2. v1 由严格 DTO 完整解码并验证后映射为 v2，再由严格 v2 DTO 顺序迁移为 v3；迁移成功前不替换 current。
3. v1/v2 已有值保留语义：既有默认模式、外观、翻页组、学习和输入辅助开关原样映射；新增字段
   使用兼容默认（v1 的五码、混输、提示、分号/单引号关闭、布局跟随系统；v3 扩展汉字关闭）。
...
- 高级设置页必须同时显示“私密模式”和“本地学习”，打开或重新打开时回显当前运行状态。
- 两项由用户点击后立即应用到全部活动会话，不等待普通 Settings Save，也不因 Cancel 回滚。
- 私密模式或关闭本地学习时，已有学习分数不参与排序且学习域零写入。
- 产品不得为两项状态创建独立状态栏项目或重复菜单；普通 Settings 保存、取消和恢复默认不得
  意外改变这两个运行时值。
