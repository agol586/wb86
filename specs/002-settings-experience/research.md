...
**Decision**: 对齐 Rime 官方 `rime-wubi` 与 librime `charset_filter`：基础词库保留完整内容；设置快照
保存默认关闭的 `extendedCJK`。运行时先对五笔和拼音分别排序并应用每类 64 项的既有安全上限，再按
当前简繁模式转换最终显示文本；关闭设置时，在显示文本去重和分页前过滤任何包含 CJK 扩展 A...J、
兼容汉字或兼容汉字补充码位的候选，开启时跳过过滤。实现使用纯 Swift 固定范围，不链接 librime。

**Rationale**: Rime 官方五笔方案使用 `extended_charset` 的“常用/增广”开关与
`enable_charset_filter: true`，其源码以 Unicode 区块而非词频权重定义扩展字符。该设计能隐藏
`wqvb` 的 `𠛈`（U+206C8，扩展 B）而不破坏完整词库，并保持跨机器可复现。

**Alternatives considered**: 删除全部 `weight=0`（会删除 42,887 条且混淆词频与字符集）；按当前
系统字体过滤（结果随机器变化）；安装字体后始终展示（目标应用仍可能缺字形且日常候选噪声不变）。
