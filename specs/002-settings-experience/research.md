...
记录真实生成资源大小及 Release 进程物理 footprint。映射资源只创建一次，页面按需解码，输入路径
不得读取设置文件或完整物化词典/学习快照。真机还要验证 flagsChanged 交付、按键透传、点击外部取消、
候选鼠标行为和多应用会话。

**Rationale**: 当前实测距 15 MB 上限余量有限，资源大小估计不能替代发布进程证据；InputMethodKit
部分行为也无法由纯单元测试证明。
