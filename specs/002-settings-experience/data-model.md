...
混输开启时，有效拼音前缀优先继续组合；第五码首选上屏只在路由已确定为五笔时触发。任一有效组合
的第一页查询无匹配时，组合保持不变，并创建正文等于当前序列、来源为 `directInput` 的不可学习候选；
该候选可被显式选择，但不触发“四码唯一”或“第五码首选”自动提交。后续字母继续追加到同一原码
组合，超过四码也不分段、提交或清空，直到查询重新产生词库候选或用户用空格、Return/Enter、数字或
鼠标显式整体提交。普通组合按 Return/Enter 提交当前原码、隐藏候选且不换行。
...
- `anchorRect`: InputMethodKit 客户端为当前 inline session 返回的完整输入行矩形。
- `visibleFrame`: 包含输入行中心点的显示器可见区域。
- `spacing`: 输入行与候选窗口之间固定至少 4 点。
- `placement`: 下方空间足够时为 `below`，否则为 `above`；最终窗口矩形不得与 `anchorRect` 相交。
