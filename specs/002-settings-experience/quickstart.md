...
- 分别检查纵向与横向候选布局：纵向每行的悬停和点击底色铺满内容宽度；首选同时显示强调底色与
  ...
  整个候选窗不得抢走当前文本焦点。
- 把输入位置移动到屏幕底部附近并开始组合；下方空间不足时，候选窗必须翻转到输入行上方，与输入
  字符至少保持 4 点间距，且窗口和阴影都不得覆盖 marked text。
- 在系统辅助功能中分别开启“减少透明度”和“增强对比度”，确认可见候选窗即时改为实色系统背景；
  增强对比度时边框更清晰，关闭后恢复系统浮层材质，深浅外观下正文与选中状态都清楚可辨。
...
## 8. Official website acceptance

1. 在 375px、768px 和桌面宽度检查官网，确认首屏、体验、隐私、三步安装和 FAQ 无横向溢出，主操作
   目标清晰，键盘 Tab 可遍历导航、链接、按钮和 FAQ，焦点始终可见。
2. 分别在系统浅色与深色外观下检查文字、卡片、候选示意和代码块对比度；关闭浏览器模糊支持时内容
   仍可辨识，页面不依赖外部字体、脚本、跟踪或 cookie。
3. 开启“减少动态效果”，确认位移动画消失但信息不缺失；触屏宽度不残留 hover 状态，精细指针下
   按钮只提供轻量、不超过 300ms 的 transform/opacity 反馈。
4. 确认 v1.5.4 Release 链接、macOS 13+、Apple Silicon、源码发布和 Developer ID 公证门禁描述与 README
