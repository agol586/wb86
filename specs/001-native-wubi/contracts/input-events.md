...
- `letter(a...y)`：有效编码字母；组合开始后的大写字母归一化。中文空闲态首个大写
  `A...Z` 是单次原样组合的开始信号，不归一化为五笔首码。
...
2. 任一有效组合的本地查询无匹配时，原码作为不可学习候选显示并可由空格或数字选择；其他真正空页的
   空格和数字选择不得提交文本。
...
9. 中英文切换必须先原子提交当前组合的原始编码，再隐藏候选并变更语言；其他模式切换按设置
   取消当前组合。每个事件不得产生两次提交。
10. 安全输入状态下所有结果的学习增量必须为空。
11. 中文空闲态首个大写字母开启单次原样组合：本段字母和普通可打印字符保留大小写与符号，
    以 marked text 和单一原文候选同步显示，不查询、不学习且不改变会话语言。空格只提交原文并由
    输入法消费，不向应用插入空格；适配器必须同时消费原始 keyDown 与 InputMethodKit
    `inputText:client:` 文本回调路径；
    回车提交原文并由输入法消费，不向应用插入换行；适配器必须同时处理原始 keyDown 与
    InputMethodKit `insertNewline:` 命令路径。首项选择提交原文；退格修正，Escape 取消。
