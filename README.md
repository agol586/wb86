...
核心输入、模式、候选、本地学习、用户词库、设置、导入导出、版本化快照、恢复、隐私和
无障碍适配已经实现，并有自动化测试。发布仍被实机 macOS 13/当前系统矩阵、人工无障碍矩阵、
八小时压力、30 天替代研究、首次用户研究以及最终 Developer ID 公证门禁阻塞；仓库当前没有
可对外发布的最终安装包。
...
| 性能 | 候选 `<2 ms`，正常输入 physical footprint `<15 MiB`，八小时无持续增长 |

Intel Mac、Rosetta 2、Universal Binary、拼音/五笔拼音、五笔 98、新世纪五笔、账户、云同步、
在线词库、遥测和广告都不属于当前范围。
...
- 本地有界学习、用户词条管理、私密模式和分域删除。
- 版本化文本/产品格式导入导出、原子快照、逐版本迁移和损坏隔离。
- 候选/按键/学习/隐私设置，VoiceOver 语义、键盘焦点和多显示器布局适配。
...
八小时压力门禁使用 `Scripts/run-long-stress.sh 28800`；已运行输入法的内存门禁使用
`Scripts/measure-memory.sh [PID]`。最终发布还必须完成
[quickstart](specs/001-native-wubi/quickstart.md)和[发布清单](Docs/ReleaseChecklist.md)，不能用
自动化 fixture 替代实机、VoiceOver、应用兼容或公证验证。
...
基础五笔词库来自固定提交的 `rime/rime-wubi`，对应源、`AUTHORS`、LGPL-3.0-only `LICENSE`、
生成 manifest 和纯 Swift 可复现编译链均随资源保存。详见
...
[规格](specs/001-native-wubi/spec.md)、[计划](specs/001-native-wubi/plan.md)和
[任务](specs/001-native-wubi/tasks.md)。按任务依赖测试先行，只有 fresh validation 和证据齐全
